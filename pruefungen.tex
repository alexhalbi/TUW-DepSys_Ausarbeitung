...
ist  \textbf{fail­
operational}), sind Safety und Reliability jedoch eng verwandt (z.B.
...
\subsection[Warum werden beim Modellieren von zuverlässigen Systemen gerne
    ...
    $\Delta t$ in Bezug auf die Anzahl der korrekten Komponenten zum Zeitpunkt
    $t$:
    ...
    werden kann, als $R(t)=e^{-\lambda t}$.
    \[f(t)=\frac{d R(t)}{dt}=-\lambda e^{-\lambda t}\]

    Das bedeutet, es ist viel einfacher zu berechnen durch eine konstante
    Fehlerrate. Außerdem können die Fehlerraten aus bekannten Bauteilen aus
    Tabellen ausgelsen werden und direkt in der Formel verwendet werden.
...
\end{figure}
    In Halbleiteranwendungen wird häufig die \textbf{Lognormal-verteilte Fehlerrate} verwendet. Diese passt sich gut an die Halbleiterlebensdauer an.
\subsection{Wahrscheinlichkeitstheoretische Modellierung was gehört dazu?}
    Simple Blockdiagram, Arbitrary\footnote{arbitrary: frei, beliebig, willkürlich} Blockdiagram, Markov Model, General Stochastic
\subsection[Simple/ Arbitrary Block Diagram, Markov Model, General Stochastic
    Petri Net?]{Simple/ Arbitrary Block Diagram, Markov Model, General Stochastic
    Petri Net?\\{\itshape\smaller[2] 7.2013, Ausarb. 2009}}
    Alle diese Modelle gehören zur strukturellen wahrscheinlichkeitstheoretischen Modellierung.
    Zur vereinfachung werden die folgenden Annahmen gemacht: die Teilkomponenten sind unterscheidbar und jede Komponente hat eine eigene Fehlerrate. Modell basiert auf den Verbindungen der einzelnen Komponenten.
    ...
    Wie Simple Block Diagram, aber die Beschränkung auf serielle und parallele Verbindungen 
fällt weg.
     \begin{figure}
    \centering
...
(abgesehen vom vorhergehenden Zustand)
    ...
    \minisec{Evaluierung}
        Aufstellen der Differentialgleichungen, anschließends lösen (z.B. mit SHARPE)
    ...
    \caption{Übergänge identischer Komponenten zusammenfassen}
...
\subsubsection{GSPN General Stochastic Petri Net}
...
\subsection[Markoveigenschaft]{Markoveigenschaft\\{\itshape\smaller[2] Ausarb. 2009}}
Das   Verhalten   des   Systems   ist   zu   jedem   Zeitpunkt   unabhängig   von   der   Vorgeschichte, 
abgesehen vom letzten Zustand.

\subsection[Modellierung für Software]{Modellierung für Software?\\{\itshape\smaller[2] 2010}}
Reliability Growth Model!!!!

Reliability Growth Model!!!!
\subsection[Reliability Growth Model?]{Reliability Growth Model?\\{\itshape\smaller[2] Ausarb. 2009}}
...
nicht modelliert werden können und Parallelität und Synchronisierung nicht beachtet werden 
(außer bei GSPNs)

Dafür sind aber Reliablility Growth Models besser geeignet:

diese basieren auf der Idee einer Iterativen Verbesserung nach dem Schema testing $\rightarrow$  correction $\rightarrow$ re­testing
...
\subsection[Assumption Coverage]{Assumption\footnote{assumption: Annahme, These, Vermutung} Coverage?\\{\itshape\smaller[2] 7.2013, Ausarb. 2009}}
    ...
    Die berechnete Reliability eines Systems muss immer noch mit der Assumption
    Coverage 
...
\subsection[Was ist Coverage allgemein und was bedeutet sie beim Markov­Graphen?]{Was ist Coverage allgemein und was bedeutet sie beim Markov­Graphen?\\{\itshape\smaller[2] Ausarb. 2009}}
...
\minisec{Assumption Coverage}
Zu welchem Prozentsatz stimmen die getroffenen Annahmen 
mit der Realität überein? (siehe \Fref{fig:actv_red})
...
Beim Markov- Graph muss die Coverage entsprechend berücksichtigt werden. D.h. die Wahrscheinlichkeit das der Fehler innerhalb der fault- semantic liegt muss bei den "normalen" Übergängen berücksichtigen. Mit der Gegenwahrscheinlichkeit führt dann zusätzlich noch eine Kante direkt zum Fehlzustand. 
...
keine   schwerwiegenden   Konsequenzen   haben,   während sehr sehr selten Fehler   mit   katastrophalen 
...
Wobei $R_0$ konstant ist, $T$ die absolute temperatur ($K$) angibt, $E_A$ die aktivierungsenergie in $eV$ angibt und $k$ die Bolzmannkonstante mit $8.6\cdot 10^{-5}eV/K$ angibt.
    \begin{figure}
    \centering
...
Wird verwendet um die Infant­Mortality­Failures schon vor der Auslieferung zu erkennen, 
...
\begin{itemize}
    ...
    \item Beschießen mit Alpha­Partikeln
\end{itemize}
...
\subsection[Safety-Analysis]{Safety-Analysis\\{\itshape\smaller[2] 7.2013, Ausarb. 2009}}
...
Falscher   Reihenfolge   durchgeführt,   etc..)   erstellt.   Darauf   aufbauend   kann   analysiert 
...
\subsection[Maintenance (warum besser als Fault­Tolerance)?]{Maintenance (warum besser als Fault­Tolerance)?\\{\itshape\smaller[2] Ausarb. 2009}}
...
Wannimmer es möglich ist, sollte Maintenance der Fehlertoleranz vorgezogen werden.
...
\end{itemize}
\minisec{Aspekte}
Wartungskosten immer mit systemkosten vergleichen, gewünschtes Wartungspersonal(Nummer, Ausbildung, Werkzeug, Wo befinden sie sich, 0-24,...)

Eigenes Ersatzteillager oder Ersatzteile erst bestellen, wenn benötigt? 

Außerdem wichtig ist die Doku, geplante tests, die systemstruktur, gute fehlermeldungen, die größe und komplexität der Ersatzteile, usw.
...
\minisec{Anwendungsspezifische Fehlertoleranz:} Der physikalische Prozess, mit dem das Computersystem interagiert unterliegt gewissen physikalischen Gesetzen und dadurch Beschränkungen. Diese Beschränkungen können im Computersystem implementiert werden um Fehler zu identifizieren. Die Plausibilitätsprüfungen sind dabei anwendungsspezifisch und ermöglichen ein "fail-stop" Verhalten des Systems. Um ein fail-operational Verhalten des Systems zu implementieren kann zusätzlich eine Abschätzung des Betriebszustandes (state-estimation) erfolgen.
...
\subsection[Systematic VS app-specific fault-tolerance?]{Systematic VS app-specific fault-tolerance?\\{\itshape\smaller[2] 7.2013}}
\minisec{Systematisch}
Replizieren der Komponenten, Abweichung der Komponenten zur Fehlererkennung, redundante Komponenten für Kontinuierlichen Service
\minisec{Application Spezifisch}
vernünftige Checks für die Fehlererkennung (basierend auf einem realen Modell), außerdem die Einschätzung für Koninuierliche Services
...
Bei diesen Systemen ist auch kenntnis der Anwendung erforderlich, bei Systematischen systemen nicht.

Bei einem systematischen System kann exat zwischen einem Fehler und korrektem Verhalten unterschieden werden. Bei Applikationsspezifischer Fehlertoleranz nicht. (Graue Zone)
...
die Servicequalität im fehlerfall bei systematischen systemen ist jedoch mit fehlerhaften replizierten Komponenten unabhängig davon, ob sie fehlerhaft sind, oder nicht.
Das korrekte system hängt ab von der anzahl der korrekt funktionierenden Replikationen und deren fehlersemantik.

Beide Systeme bieten Backward recovery, aber das Application Specific system bietet auch forward recovery. Auch ist das Application Specific system günstiger, was die anzahl der komponenten angeht, denn hier fallen keine kosten für die redundanz an.
...
Es scheint, dass Computer deterministisch arbeiten, aber in wahrheit arbeiten sie großteils deterministisch.
...
\item unterschiedliche   Wahrnehmung   der  Reihenfolge 
unterschiedliche Message Transmission Delays
\item Inkonsistente Information über die Gruppenmitgliedschaft von Replikationen
\item nichtdeterministische  Programmkonstrukte 
Kommunikation, Synchronisation, zufällige Zahlen, etc.)
...
\item Dynamische Scheduling­Entscheidungen
\item Verzögerungen beim Übertragen von nachrichten.
\item Unterschiedliche Rundung von Real­Zahlen auf unterschiedlichen Implementierungen dabei können unterschiedliche impplementierungen zu auseinandergehenden resultaten führen, wenn die arithmetischen Berechnungen nahe der Grenzen der Äquivalenzklassen liegen.
\end{itemize}
...
Ein Prozessor ist synchron, wenn er mindestens einen Prozessschritt während $\Delta$ real-time steps.
\minisec{Kommunikation begrenzt}
Die Kommunikationsverzögerung heißt verzögert, wenn eine Nachricht, die geschickt wurde nach höchstens $\Phi$-Schritten am Ziel ankommt (wenn kein Fehler passiert).
...
\subsection[Consensus Protocols?]{Consensus Protocols?\\{\itshape\smaller[2] 7.2013, Ausarb. 2009}}
...
Asynchrone systeme können keinen consensus durch einen deterministischen Algorithmus erreichen.

Da es unmöglich ist, zwischen einer späten Antwort und einem Prozessorcrash zu unterscheiden.
...
Fail­Silent: Eine fehlerhafte Komponente erzeugt keine Ausgabe

Fail­Consistent: Eine fehlerhafte Komponente erzeugt falsche Ausgaben, diese werden von 
allen Komponenten gleich wahrgenommen.

Byzantinisch: Eine fehlerhafte Komponente erzeugt falsche Ausgaben, diese werden von 
allen Komponenten unterschiedlich wahrgenommen.
...
\begin{description}
    ...
    \item [systemstruktur] ungünstige Systemstruktur (unpassende komponenten) 
        kann zu wenig vertrauen führen.
    \item [low dependability of components] das system ist so stark wie das
        schwächste Glied.
...
\minisec{Stresstests}
Function Test, Heat Test, Cold Test, Heat storage, Temperature shock, \ldots
\subsubsection{Beeinträchtigung der Zuverlässigkeit(Dependability):}
\minisec{Ausfall (failure)}
Die Einhaltung der Servicespezifikation wird erschwert. (Übergang von korrekten
zu inkorrektem Service)
\minisec{Fehlerzustand (error)}
Teil eines Systemzustands, der evtl. zu einem Fehler führen kann.
\minisec{Fehlerurache (fault)}
Fehlerursache, die toleriert oder verhindert werden soll.
...
\begin{figure}
    ...
    \scalebox{0.8}{
        \begin{tikzpicture}[>=latex]
            ...
            align=center]{fault\\activation}(c);
...
\begin{description}
    ...
    \item [Fehlertoleranz]: Methoden und Techniken die Services zur verfügung
